...
{
    public class RepositoryNHEsolPortal : IRepositoryNHEsolPortal
    {
        private readonly IUnitOfWorkNHEsolPortal _unitOfWork;
        ...
        public RepositoryNHEsolPortal(IUnitOfWorkNHEsolPortal unitOfWork,
        ...
        public async Task<T> Save<T>(T entity)
        {
            ...
            if (entity is EntityBasePortal objEntity)
            {
        ...
        public async Task<IList<T>> SaveRange<T>(IList<T> entities)
        {
            ...
            var usuario = await _authenticatedBaseService.GetLoggedUserAsync();
            foreach (var entity in entities.Cast<EntityBasePortal>())
            {
                if (entity is IEntityValidateCore validate)
                    await validate.SaveValidate();
        ...
        public async Task<IList<T>> FindBySql<T>(string sql, int pageSize, int pageNumber, params Parameter[] parameters)
        {
            ...
            var sqlBase = NormalizaParameterName(sql, parameters);

            sql = PaginationHelper.GetPaginatedQuery(sqlBase, DataBaseType, pageNumber, pageSize);
        ...
        public async Task<Int64> CountTotalEntry(string sql, params Parameter[] parameters)
        {
            ...
            var sqlPronto = $"Select COUNT(1) FROM ({sqlBase}) a  ";
        ...
        {
            throw new NotImplementedException("GetParametroSistemaViewModel não está implementado para RepositoryNHEsolPortal");
        }
