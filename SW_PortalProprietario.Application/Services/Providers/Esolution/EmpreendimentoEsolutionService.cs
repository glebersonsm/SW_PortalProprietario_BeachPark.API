...
{
    public class EmpreendimentoEsolutionService : IEmpreendimentoProviderService
    {
        ...
        private readonly ILogger<EmpreendimentoEsolutionService> _logger;
        ...
        private readonly IFinanceiroProviderService _financeiroService;
        private readonly IServiceBase _serviceBase;
        private readonly IEmailService _emailService;
        private readonly ICacheStore _cacheStore;

        private string CommunicationProviderName => "EsolutionProvider";
        public EmpreendimentoEsolutionService(ILogger<EmpreendimentoEsolutionService> logger,
            ...
            IFinanceiroProviderService financeiroService,
            ICacheStore cache)
        {
            _logger = logger;
        ...
        public async Task<bool> GerarCodigoVerificacaoLiberacaoPool(int agendamentoId)
        {
            var loggedUser = await _repositorySystem.GetLoggedUser();
        ...
        public async Task<bool> ValidarCodigo(int agendamentoId, string codigoVerificacao, bool? controlarTransacao = true)
        {
            try
            {
        ...
        public async Task<ResultWithPaginationModel<List<Models.Empreendimento.SemanaModel>>?> ConsultarAgendamentosGerais(ReservasMultiPropriedadeSearchModel model)
        {
            var dadosCache = await _cacheStore.GetAsync<List<ContratoSCPModel>>($"{CACHE_CONTRATOSSCP}");
            ...
            var inadimplentes = model.ApenasInadimplentes.GetValueOrDefault(false) ? await Inadimplentes() : new();
        ...
        public async Task<ResultWithPaginationModel<List<Models.Empreendimento.SemanaModel>>?> ConsultarMeusAgendamentos(PeriodoCotaDisponibilidadeUsuarioSearchModel model)
        {
        ...
        public async Task<Models.ResultModel<List<InventarioModel>>?> ConsultarInventarios(InventarioSearchModel searchModel)
        {
        ...
        public async Task<Models.ResultModel<bool>?> RetirarSemanaPool(AgendamentoInventarioModel modelAgendamentoPool)
        {
            ...
            try
            {
                ...
                if (modelAgendamentoPool.InventarioId.GetValueOrDefault(0) == 0)
                {
                    ...
                    Models.ResultModel<List<InventarioModel>>? inventariosVinculadosAoAgendamentoNoPool = await ConsultarInventarios(new InventarioSearchModel()
                    {
        ...
        public async Task<ResultModel<bool>?> LiberarMinhaSemanaPool(LiberacaoMeuAgendamentoInputModel modelAgendamentoPool)
        {
            ...
            catch (Exception err)
            {
                ...
                {
                    var result = await RetirarSemanaPool(new AgendamentoInventarioModel()
                    {
                        AgendamentoId = modelAgendamentoPool.AgendamentoId,
                        AcaoInterna = true
                    });
            ...
            int empresaId, bool espanhol = false)
        {
            ...
            var dadosPreenchimentoContrato = await GetKeyValueListFromContratoSCP(new GetHtmlValuesModel()
            {
        ...
        {
            ResultModel<List<InventarioModel>>? inventariosVinculadosAoAgendamentoNoPool = await ConsultarInventarios(new InventarioSearchModel()
            {
                Agendamentoid = modelAgendamentoPool.AgendamentoId,
                NoPool = "S"
            });
            ...
            {
                bool codigoValido = await ValidarCodigo(modelAgendamentoPool.AgendamentoId.GetValueOrDefault(), modelAgendamentoPool.CodigoVerificacao, false);
                if (!codigoValido)
        ...
        public async Task<ResultModel<List<SemanaDisponibilidadeModel>>?> ConsultarDisponibilidadeCompativel(DispobilidadeSearchModel searchModel)
        {   
        ...
        public async Task<List<KeyValueModel>> GetKeyValueListFromContratoSCP(GetHtmlValuesModel model, string codigoEnviadoAoCliente, DateTime? dataAssinatura, bool espanhol = false)
        {
        ...
        public async Task<DownloadContratoResultModel?> DownloadContratoSCP(int cotaId)
        {
            try
            {
                _repositorySystem.BeginTransaction();
        ...
        public async Task<ResultModel<int>?> TrocarTipoUso(TrocaSemanaInputModel model)
        {
            ResultModel<int> retornoVinculo =
            new ResultModel<int>(-1)
            {
        ...
        public async Task<List<Models.Empreendimento.StatusCrcModel>?> ConsultarStatusCrc()
        {
        ...
        public async Task<List<PosicaoAgendamentoViewModel>> GetPosicaoAgendamentoAnoAsync(int ano, int? uhCondominioId = null, int? cotaPortalId = null)
        {
            List<Parameter> parameters = new List<Parameter>();
