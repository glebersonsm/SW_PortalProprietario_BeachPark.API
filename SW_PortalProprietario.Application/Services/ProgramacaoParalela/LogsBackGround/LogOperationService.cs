...
namespace SW_PortalProprietario.Application.Services.ProgramacaoParalela.LogsBackGround
{
    public class LogOperationService : ILogOperationService
    {
        ...
        public LogOperationService(IServiceScopeFactory scopeFactory,
            ...
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                using (var session = repository.CreateSession())
                {
                    ...
                    try
                    {
                        ...
                        var logAcessoResult = await repository.ForcedSave(logAcesso, session);
                        ...
                        if (log.Modificacoes != null && log.Modificacoes.Any())
                        {
                            foreach (var itemModificado in log.Modificacoes)
                            {
                                ...
                                var logAcessoObjetoResult = await repository.ForcedSave(logAcessoObjeto, session);
                                ...
                                foreach (var itemCampo in itemModificado.Modificacoes)
                                {
                                    ...
                                    var logAcessoObjetoCampoResult = await repository.ForcedSave(logAcessoObjetoCampo, session);
                                }
                            }
                        }

                        var resultCommit = await repository.CommitAsync(session);
