...
{
    public static class NHibernateHostedServiceExtensions
    {
        public static IServiceCollection AddNHbernateHosted(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            if (!string.IsNullOrEmpty(connectionString))
            {
                ...
                {
                    CreateSqLiteConnection(services, connectionString);
                }
        ...
        private static void ConfigureOracleConnection(IServiceCollection services, IConfiguration configuration, string? connectionString)
        {
            ...
            var sf = new SwSessionFactoryHosted() { SessionFactory = sessionFactory };

            services.TryAddSingleton<ISwSessionFactoryHosted>(sf);
            services.TryAddSingleton(factory => sf.OpenStatelessSession());
        }

        private static void CreateSqLiteConnection(IServiceCollection services, string? connectionString)
        {
            var _sessionFactory = Fluently.Configure()
