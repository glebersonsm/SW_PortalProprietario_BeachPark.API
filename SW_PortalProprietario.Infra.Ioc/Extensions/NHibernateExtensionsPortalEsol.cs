...
{
    public static class NHibernateExtensionsPortalEsol
    {
        public static IServiceCollection AddNHbernatePortalEsol(this IServiceCollection services, IConfiguration configuration)
        {
            ArgumentNullException.ThrowIfNull(services, nameof(services));
            ArgumentNullException.ThrowIfNull(configuration, nameof(configuration));

            var connectionString = System.Environment.GetEnvironmentVariable("ESOL_PORTAL_CONNECTION");
            if (string.IsNullOrEmpty(connectionString))
            {
                throw new ArgumentException("Não foi configurada a string de conexão ESOL_PORTAL_CONNECTION.");
            }
        ...
        private static void ConfigureOracleConnection(IServiceCollection services, string connectionString)
        {
            var _sessionFactory = Fluently.Configure()
                    ...
                    .Raw("throw_on_error", "true"))
                        .Mappings(m => m.FluentMappings.AddFromAssemblyOf<EsolutionPortalDomain.AutomappingConfigurationEsolutionPortal>());

            var sessionFactory = _sessionFactory.BuildSessionFactory();
            var sf = new SwPortalSessionFactory() { SessionFactory = sessionFactory };

            services.TryAddSingleton<ISwPortalSessionFactory>(sf);
