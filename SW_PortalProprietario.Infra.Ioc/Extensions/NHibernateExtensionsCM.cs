...
namespace SW_PortalProprietario.Infra.Ioc.Extensions
{
    ...
    {
        public static IServiceCollection AddNHbernateCM(this IServiceCollection services, IConfiguration configuration)
        {
        ...
        private static void CreateSqlServerConnection(IServiceCollection services, string connectionString)
        {
            ...
            .Mappings(m =>
              m.FluentMappings.AddFromAssemblyOf<AutomappingConfigurationCm>());

            var sessionFactory = _sessionFactory.BuildSessionFactory();
            var sf = new SwCmSessionFactory() { SessionFactory = sessionFactory };

            services.TryAddSingleton<ISwCmSessionFactory>(sf);
        ...
        private static void CreatePostgreSqlConnection(IServiceCollection services, string connectionString)
        {
            var _sessionFactory = Fluently.Configure()
                        ...
                        .Mappings(m => m.FluentMappings.AddFromAssemblyOf<AutomappingConfigurationCm>());
